...
namespace EVRPlayer
{
    ...
    class DShowPlayer : COMBase, IDisposable
    {
        ...
        public int S_Ok { get; private set; }
        public Exception E_InvalidArgument { get; private set; }
        public Exception E_Pointer { get; private set; }
        public Exception E_Unexpected { get; private set; }
        ...
        public DShowPlayer(Control hwndVideo, IntPtr hwnd, int msg)
        {
            ...
            // m_clsidPresenter = Guid.Parse("1643E180-90F5-11CE-97D5-00AA0055595A");            
        }
        ~DShowPlayer()
        {
            Dispose();
      ...
      if (classEnum.Next (moniker.Length, moniker, IntPtr.Zero) == 0)
        ...
        public void HandleGraphEvent(GraphEventCallback pCB)
        {
            ...
            while (Succeeded((HResult)m_pEvent.GetEvent(out evCode, out param1, out param2, 0)))
            {
                // Invoke the callback.
        ...
        private int ConnectFilters(IBaseFilter filter1,IBaseFilter filter2)
        {
            IPin pPinIn= DsFindPin.ByConnectionStatus(filter1, PinConnectedStatus.Unconnected, 0);
        ...
        private void RenderWebCam(Control control,IBaseFilter pSource)
        {
            ...
            IBaseFilter pEVR = (IBaseFilter)new EnhancedVideoRenderer();
            IBaseFilter pMPEG = (IBaseFilter)new MjpegDec();
            IBaseFilter pColor = (IBaseFilter)new Colour();
            try
            {
                ICaptureGraphBuilder2 cap;
                cap= (ICaptureGraphBuilder2)new CaptureGraphBuilder2();
                hr = m_pGraph.AddFilter(pMPEG, "MPEG Decompresser");
                DsError.ThrowExceptionForHR(hr);

                hr=ConnectFilters(pSource,pMPEG);

                hr = m_pGraph.AddFilter(pColor, "Color Space");
                DsError.ThrowExceptionForHR(hr);

                hr = ConnectFilters(pMPEG,pColor);
                ...
                try
                {
                    ...
                    hr = cap.RenderStream(null, MediaType.Video, pColor, null, pEVR);
                    DsError.ThrowExceptionForHR(hr);
